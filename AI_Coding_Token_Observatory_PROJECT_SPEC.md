...
总览页顶部工具栏对齐修复已完成（2026-08-13）：共享主导航与采集状态、扫描全部来源、本地网页操作现在位于同一个响应式工具栏中；宽窗口下左右分列且垂直居中，消除左右两侧错行，窄窗口下按导航在前、操作区在后的顺序换为两行。其他路由继续保持导航位于内容区左上角。新增 DOM 结构回归断言，确保导航和操作按钮属于同一总览工具栏。

额度页导航基线统一已完成（2026-08-13）：额度路由移除额外的 `quota-page` 外层和桌面/窄窗口专属顶部 padding 覆盖，直接复用与会话、Providers、数据源、设置相同的 `PageFrame` / `.app-shell` 页面根节点与顶部间距；切换到额度时主导航不再向窗口顶部跳动。额度卡片的自然高度、账号、快照和刷新交互保持不变。新增额度路由结构回归断言，防止再次引入独立页面顶距。

DeepSeek Harness 数据源适配与面板 UI 优化已完成（2026-08-13）：新增独立 `tokenbuddy-dsh-session`，只读解析 DeepSeek Harness 会话工件（默认 `<DSH_HOME>/sessions/**/session.jsonl`，其中 `DSH_HOME` 为 `$DSH_HOME` 或 `~/.dsh`；同时支持 Zstandard 逐帧压缩的 `session.jsonl.zstd` 与明码编码，两种编码共享同一逻辑资源身份）。头部行（session id、cwd、parentSession、origin、delegationDepth）映射为会话与父子关系；`assistant/message` 的 `usage`（`inputTokens`/`outputTokens`/`cacheReadTokens`/`cacheWriteTokens`/`reasoningTokens`）按 DSH 的分离计数语义映射为统一 Token 语义（缓存字段缺失时总输入取上报值、未缓存拆分保持 `Unavailable`，绝不虚构）；`request/context` 与 `request/header` 携带的 provider/model 作为路由上下文生成 Provider 记录与模型归属；主/子代理经 `origin`/`delegationDepth` 区分。精度为 `ExactSession`，Prompt/completion/推理正文与工具参数从不进入领域模型，`raw_usage_json` 仅存数值。增量导入以文件 size+mtime 跳过未变文件、整文件重解压后按明文字节偏移续读，截断/轮转（头部 id 变化）自动重置，未完成尾行保留重试，坏行计数跳过；`AppKind` 新增 `deepseek_harness`（SQLite 迁移 0011 新增 `dsh_home` 设置列）。Core、Tauri commands、loopback `/api/detect-dsh`、`/api/rescan-dsh`、设置页与总览筛选均已接入。真实本机 DSH 会话冒烟验证头部与 usage 字段名一致（未入库）。UI 同步优化：设置页补齐“数据保留周期（天）”与 DSH 路径字段，总览数据源区新增 DSH 检测行并更新说明，会话页增加计数行，数据源页健康状态彩色徽章，Providers 页补缓存命中率，会话行与按模型表格的应用名本地化显示。新增适配器 11 项单测、Core 3 项集成测试与前端 3 项回归测试；Rust fmt/clippy/测试（desktop 链接除外）、前端格式/lint/61 项测试与生产构建均通过。当前限制：仅支持 `SESSION_FORMAT_VERSION = 0`，未来格式版本会跳过并计坏行；单文件解压上限 256 MiB；Windows 托盘等真机验收仍归入未完成的 `Phase 4b 跨平台真机交互补验`。

TokenBuddy v0.1.7 发布已完成（2026-08-13）：桌面端、Tauri 与 Rust workspace 版本统一升级至 `0.1.7`，锁文件同步；`v0.1.7` tag 通过既有 Release 工作流在 Windows/macOS 目标系统构建签名安装包（NSIS/MSI/DMG）、更新 `latest.json` 并发布正式 Release，0.1.6 用户获得应用内更新。本版本包含 Web API 防 CSRF/DNS-rebinding 与依赖漏洞升级（e13da6e）以及 DeepSeek Harness 数据源与面板 UI 优化（db38c31）。

Windows 托盘菜单死锁修复已完成（2026-08-14，v0.1.8）：Windows 真机反馈右键托盘菜单「打开完整面板」无效且触发后应用死锁。定位为 Windows 托盘上下文菜单在主线程序列运行模态循环，菜单回调内同步创建/显示 WebView2 窗口导致主线程死锁（死锁实例全部窗口消息超时、CPU 0%，单实例转发的同一路径因走普通消息分发而不受影响）。修复为新增 `deferred_on_main`：托盘菜单、托盘左键/双击与单实例转发统一把窗口创建/显示推迟到下一主循环迭代（`thread::spawn` + `run_on_main_thread`），脱离菜单模态上下文；并新增 `TOKENBUDDY_LOG_FILE` 环境变量门控的窗口构建失败诊断日志。本机 Windows 真机（MSVC debug 构建 + 托盘 UI 自动化）验证：菜单「打开完整面板」2 秒内窗口可见且消息循环响应正常、「打开快速摘要」窗口可见、第二实例转发正常、全程无死锁；`cargo fmt`/`clippy -D warnings` 与 `cargo test --workspace --all-targets`（MSVC，含 desktop MockRuntime 测试）全部通过。版本升级至 `0.1.8` 并发布，0.1.7 用户可通过应用内更新或重新安装修复。Windows 托盘其余真机场景（自启动、高 DPI、隐藏窗口持续采集）仍归入未完成的 `Phase 4b 跨平台真机交互补验`。
